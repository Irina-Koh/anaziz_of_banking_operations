...
def get_cards_with_spend(sorted_df: DataFrame) -> list[dict]:
    """
    ...
    for index, row in card_sorted.iterrows():
        if row["Сумма операции"] < 0:
            ...
            transaction_row = {"last_digits": last_digits, "total_spent": total_spent, "cashback": cashback}
...
def get_top_transactions(sorted_df: pd.DataFrame, get_top: int):
    ...
    try:
        ...
        top_pay_transations_sorted = top_transations[["Дата платежа", "Сумма операции", "Категория", "Описание"]]
    ...
    try:
        ...
        with open(path_to_json, "r", encoding="utf-8") as file:
            ...
            for currency in currencies:
                ...
                currency_rates.append({"currency": currence_code_response, "rate": currence_amount})
    ...
    try:
        ...
        with open(path_to_json, "r", encoding="utf-8") as file:
            ...
            for stock in stocks:
                ...
                params = {"function": "TIME_SERIES_DAILY", "symbol": stock, "apikey": API_KEY_2, "datatype": "json"}
                ...
                if status_code != 200:
                    logger.error(
                        f"Ошибка при загрузке данных для акции {stock}: "
                ...
                if closing_price is None:
                    logger.error(
                        f"Не найдена цена закрытия для акции {stock} на дату {date_str}. "
                ...
                stock_rates.append({"stock": stock, "price": stocks_amount})
        ...
        logger.error(
            f"Ошибка декодирования JSON-файла '{path_to_json}'. Возможно, файл поврежден или имеет неправильный формат."
        )
